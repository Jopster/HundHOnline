...
function toArticleRow(article) {
  return [
    ...
    article.description,
    article.supplier,
    formatNumber(article.quantity, 0),
    formatNumber(article.sales_price),
    '0,00',
    formatNumber(article.vat_rate, 0),
    article.unit,
  ];
}
...
async function loadArticles() {
  try {
    ...
    renderArticles();
    statusMessage.textContent = 'Artikeldaten wurden aus der Datenbank geladen.';
  } catch (error) {
    console.error('Artikel konnten nicht geladen werden.', error);
    statusMessage.textContent = 'Datenbank nicht erreichbar. Beispieldaten werden angezeigt.';
  }
}
